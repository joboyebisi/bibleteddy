...
/** HTML page: promotes #hash params to ?query so the server can read code / yvp_id */
function hashBootstrapHtml() {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <title>Signing in with YouVersion…</title>
  <style>
    body { font-family: system-ui, sans-serif; display: flex; align-items: center; justify-content: center; min-height: 100vh; margin: 0; background: #fbf9f5; color: #544600; }
    p { font-size: 1.1rem; }
  </style>
</head>
<body>
  <p>Completing YouVersion sign in…</p>
  <script>
    (function () {
      var url = new URL(window.location.href);
      var hash = new URLSearchParams(url.hash.replace(/^#/, ""));
      var keys = ["code", "yvp_id", "user_email", "user_name", "profile_picture", "scope", "error", "error_description"];
      var changed = false;
      ...
      // Nothing in hash — ask server to continue with state-only (Auth Call 2 retry)
      if (url.searchParams.has("state")) {
        url.searchParams.set("yv_continue", "1");
        window.location.replace(url.toString());
        return;
      }
      window.location.replace("/onboarding/signup?error=no_auth_code");
    })();
  </script>
</body>
</html>`;
...
export async function GET(request) {
  ...
  const yvContinue = searchParams.get("yv_continue");

  // First pass: only state — check URL hash client-side, or retry Auth Call 2
  if (!searchParams.get("code") && !searchParams.get("yvp_id") && returnedState && !yvContinue) {
  ...
  try {
    // Hop 1: user profile from YouVersion → Auth Call 2
    if (!authCode && yvpId) {
      const cookieOpts = {
        httpOnly: true,
    ...
    // State-only retry after hash bootstrap — YouVersion may have session from consent
    if (!authCode && yvContinue) {
      ...
      return NextResponse.redirect(`${authBase}/callback?state=${encodeURIComponent(returnedState)}`);
    }

    if (!authCode) {
      console.error("YouVersion callback params:", Object.fromEntries(searchParams));
      clearOAuthCookies(cookieStore);
      return NextResponse.redirect(`${origin}/onboarding/signup?error=no_auth_code`);
    }
